...
var mountDirectory = function(connect, dir) {
    return serveIndex(require("path").resolve(dir), {
        icons: true
    });
};
...
module.exports = function(grunt) {
    ...
    grunt.registerTask("adbpull", "Perform all the adbpull tasks", [
        "adbpull-debug",
        "adbpull-db",
        "adbpull-csv"
    ]);
    ...
    grunt.registerTask("adbpush", "Perform all the adbpush tasks", [
        "adbpull-props",
        "remove-folders",
        "adbpush-collect",
        "adbpush-default-app",
    ...
    grunt.registerTask("clean", "wipe the device", [
        "adbpull-props",
        "remove-folders",
        "adbpush-props",
        "setup"
    ...
    grunt.registerTask("adbpull-db", "Pull the db from the device", function() {
    ...
    grunt.registerTask(
        ...
        function() {
            var platform = require("os").platform();
            var isWindows =
                platform.search("win") >= 0 && platform.search("darwin") < 0;
            ...
            dirs.forEach(function(fileName) {
                ...
                grunt.log.writeln(
                    "macGenConvert: " + xlsFile + " > " + formDefFile
                );
                grunt.task.run(
                    "exec:macGenConvert:" + xlsFile + ":" + formDefFile
                );
            });
        }
    );
    ...
    grunt.registerTask(
        ...
        function() {
            ...
            zipAllFiles(
                buildDir + "/survey/systemzip",
                surveySystemZipFiles,
                function(outcome) {
                    if (outcome) {
                        zipAllFiles(
                            buildDir + "/survey/configzip",
                            surveyConfigZipFiles,
                            function(outcome) {
                                if (outcome) {
                                    zipAllFiles(
                                        buildDir + "/tables/systemzip",
                                        tablesSystemZipFiles,
                                        function(outcome) {
                                            if (outcome) {
                                                zipAllFiles(
                                                    buildDir +
                                                        "/tables/configzip",
                                                    tablesConfigZipFiles,
                                                    function(outcome) {
                                                        if (outcome) {
                                                            grunt.log.writeln(
                                                                "success!"
                                                                ...
                                                                "failing on /tables/configzip with error: " +
                                                                    outcome
                                                            );
                                                        }
                                                    }
                                                    ...
                                                    "failing on /tables/systemzip with error: " +
                                                        outcome
                                                );
                                            }
                                        }
                                        ...
                                        "failing on /survey/configzip with error: " +
                                            outcome
                                    );
                                }
                            }
                            ...
                            "failing on /survey/systemzip with error: " +
                                outcome
                        );
                    }
                }
    ...
    var infixRenameCopier = function(demoInfix, offsetDir) {
        return function(fileName) {
            ...
            while (idx >= 0) {
                ...
                isInfixed = true;
                destFileName =
                    destFileName.substring(0, idx) +
                    destFileName.substring(idx + demoInfix.length);
            ...
            while (idxDir >= 0) {
                ...
                isInfixed = true;
                destFileName =
                    destFileName.substring(0, idxDir) +
                    destFileName.substring(idxDir + demoInfix.length);
                idxDir = destFileName.indexOf(demoInfix + "/");
            }

            var buildDir = "build" + "/" + demoInfix.substring(1);
    ...
    grunt.registerTask(
        ...
        function() {
            ...
            // Now push these files to the phone.
            dirs.forEach(
                infixRenameAdbPusher(".largeDataSet500", tablesConfig.appDir)
            );
        }
    );
    ...
    grunt.registerTask(
        ...
        function() {
            ...
            // Now push these files to the phone.
            dirs.forEach(
                infixRenameAdbPusher(".largeDataSet3000", tablesConfig.appDir)
            );
        }
    );
    ...
    grunt.registerTask(
        ...
        function() {
            ...
            // Now push these files to the phone.
            dirs.forEach(
                infixRenameAdbPusher(".gatesDemo072216", tablesConfig.appDir)
            );
        }
    );
    ...
    grunt.registerTask(
        ...
        function() {
            ...
            // Now push these files to the phone.
            dirs.forEach(
                infixRenameCopier(".gatesDemo072216", tablesConfig.appDir)
            );
    ...
    grunt.registerTask(
        ...
        function() {
            ...
            // Now push these files to the phone.
            dirs.forEach(
                infixRenameAdbPusher(".simpledemo", tablesConfig.appDir)
            );
        }
    );
    ...
    grunt.registerTask(
        ...
        function() {
            ...
            // Now push these files to the phone.
            dirs.forEach(
                infixRenameAdbPusher(".rowlevelaccessdemo", tablesConfig.appDir)
            );
        }
    );
    ...
    grunt.registerTask(
        ...
        function() {
            ...
            // Now push these files to the phone.
            dirs.forEach(
                infixRenameCopier(".rowlevelaccessdemo", tablesConfig.appDir)
            );
    ...
    grunt.registerTask(
        ...
        function() {
            ...
            // Now push these files to the phone.
            dirs.forEach(
                infixRenameAdbPusher(
                    ".georowlevelaccessdemo",
                    tablesConfig.appDir
                )
            );
        }
    ...
    grunt.registerTask(
        ...
        function() {
            ...
            // Now push these files to the phone.
            dirs.forEach(
                infixRenameCopier(".georowlevelaccessdemo", tablesConfig.appDir)
            );
    ...
    grunt.registerTask(
        ...
        function() {
            ...
            dirs.forEach(
                infixRenameAdbPusher(".tablesdemo", tablesConfig.appDir)
            );
        }
    );
    ...
    grunt.registerTask(
        ...
        function() {
            ...
            dirs.forEach(
                infixRenameAdbPusher(".opendatakit-2", tablesConfig.appDir)
            );
        }
    );
    ...
    grunt.registerTask(
        ...
        function() {
            ...
            // Now push these files to the phone.
            dirs.forEach(
                infixRenameCopier(".opendatakit-2", tablesConfig.appDir)
            );
    ...
    // tableId.
    grunt.registerTask("addtable", "Adds a table directory structure", function(
        tableId
    ) {
        if (arguments.length !== 1) {
            ...
            var replaceIdAndWrite = function(srcPath, destPath, tableId) {
                ...
                contents = contents.replace(regex, tableId);
                grunt.file.write(destPath, contents);
            };

            grunt.log.writeln(this.name + " making table with id " + tableId);
            ...
            var toCopy = ["forms/", "collect-forms/", "instances/"];
            ...
            // table id.
            var detailHtml = {
                srcPath: tablesConfig.tableTemplateDir + "/html/detail.html",
            ...
            var filesToReplace = [detailHtml, detailJs, listHtml, listJs];
            grunt.log.writeln(this.name + ", writing dynamic table files");
            filesToReplace.forEach(function(element) {
                replaceIdAndWrite(element.srcPath, element.destPath, tableId);
            });
        }
    });
    ...
    grunt.registerTask("server", function(target) {
        if (target === "test") {
            return grunt.task.run(["connect:test", "watch:livereload"]);
        }
    ...
    grunt.registerTask(
        ...
        function killall() {
            ...
            for (var i = 0; i < apps.length; i++) {
                ...
                grunt.task.run(
                    "exec:adbshell:am force-stop org.opendatakit.".concat(
                        apps[i]
                    )
                );
            }
    ...
    grunt.registerTask(
        ...
        function remove_folders() {
            ...
            for (var i = 0; i < apps.length; i++) {
                ...
                grunt.task.run(
                    "exec:adbshell:pm uninstall org.opendatakit.".concat(
                        apps[i]
                    )
                );
            }
    ...
    grunt.registerTask(
        ...
        function remove_folders() {
            ...
            var folders = [
                tablesConfig.deviceMount + "/" + tablesConfig.appName
            ]; //, "/sdcard/odk"];
    ...
    grunt.registerTask(
        ...
        function props() {
            var base = tablesConfig.deviceMount.concat(
                "/",
                tablesConfig.appName
            );
            var destbase = tablesConfig.appDir.concat(
                "/",
                tablesConfig.outputPropsDir,
                "/"
            );
            var files = [
                "/config/assets/app.properties",
    ...
    grunt.registerTask(
        ...
        function props() {
            var file = tablesConfig.appDir.concat(
                "/",
                tablesConfig.outputPropsDir,
                "/device.properties"
            );
            ...
            for (var i = 0; i < props.length; i++) {
                if (
                    props[i].indexOf("tool_last_initialization_start_time") >= 0
                ) {
                    props[i] = "";
                }
            }
    ...
    grunt.registerTask("setup", "Launch the login and sync screen", function() {
        grunt.task.run(
            "exec:adbshell:am start -a android.intent.action.MAIN -n org.opendatakit.services/.sync.actions.activities.SyncActivity --es appName " +
                tablesConfig.appName +
                " --es showLogin true"
        );
    });
    ...
    var deleteTopLevel = function(grunt, dirToDelete) {
        var toBeDeleted = grunt.file.expand({ cwd: dirToDelete }, "*");
    ...
    grunt.registerTask(
        ...
        function() {
            ...
            deleteTopLevel(
                grunt,
                tablesConfig.appDir + "/" + tablesConfig.outputCsvDir
            );
            deleteTopLevel(
                grunt,
                tablesConfig.appDir + "/" + tablesConfig.outputDbDir
            );
            deleteTopLevel(
                grunt,
                tablesConfig.appDir + "/" + tablesConfig.outputDebugDir
            );
            deleteTopLevel(
                grunt,
                tablesConfig.appDir + "/" + tablesConfig.outputPropsDir
            );

            var assetFilesToDelete = grunt.file.expand(
                { filter: "isFile", cwd: tablesConfig.assetsDir },
                "*.*",
                "!favicon.ico",
                "css/**",
                "!css/odk-survey.css",
            ...
            var cleanFilesToDelete = grunt.file.expand(
                { cwd: tablesConfig.assetsDir, filter: "isFile" },
                "**/*.clean.*",
                "**/*.clean/**"
            );
            ...
            deleteFilesList(
                grunt,
                tablesConfig.assetsDir + "/framework/forms",
                assetsDirToDelete
            );
        }
    );
